...
class NNDT:
    ...
    def __init__(self, train_data, test_data, cut_per_feature = 1, num_bin = None, beta = 0.01,
                temperature = 100, regularizer = False, batch_size = 100, epochs = 100, model_type="Tree",*args, **kwargs):
        ...
        self.cut_points_list = [tf.Variable(tf.random_uniform([self.cuts])) for i in xrange(self.X_train.shape[1])]
        self.leaf_score = tf.Variable(tf.random_uniform([(self.cuts+1)**self.X_train.shape[1], self.y_train.shape[1]]))

        self.W = tf.reshape(tf.linspace(1.0, self.cuts + 1.0, self.cuts + 1), [1, -1])

        if self.model_type == "Tree":
            for cut_points in self.cut_points_list:
              cut_points = -tf.nn.top_k(-cut_points, self.cuts)[0]
              self.num_bin.append(tf.cumsum(tf.concat([tf.constant(0.0, shape=[1]), -cut_points],0)))
            #self.bins = tf.stack(self.num_bin)
    ...
    def nn_decision_tree(self, x, cut_points_list, leaf_score):
        ...
        return tf.sparse_matmul(reduce(self.tf_kron_prod, map(lambda z: self.tf_bin(x[:, z[0]:z[0] + 1], z[1]), enumerate(cut_points_list))), leaf_score, a_is_sparse=True)
        #return tf.matmul(reduce(self.tf_kron_prod, map(lambda z: self.tf_bin(x[:, z[0]:z[0] + 1], z[1]), enumerate(cut_points_list))), leaf_score)
    ...
    def fit_tree(self):
        ...
        y_pred = self.nn_decision_tree(x_ph, self.cut_points_list, self.leaf_score)


        if self.regularizer == True:
            loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits=y_pred, labels=y_ph) +  self.beta * tf.nn.l2_loss(self.leaf_score) + self.beta * tf.nn.l2_loss(tf.stack(self.num_bin)))
        else:
    ...
    def fit_forrest(self, max_features):

        num_trees = int(self.X_train.shape[1]/max_features)
        ...
        for i in xrange(num_trees):
            ...
            for epoch in range(self.epochs):
                ...
                total_batch = int(train_x.shape[0]/self.batch_size)

                for i in range(total_batch):

                    batch_mask = np.random.choice(train_x.shape[0], self.batch_size)


                    batch_x = train_x[batch_mask].reshape(-1, train_x.shape[1])
                    batch_y = train_y[batch_mask].reshape(-1, train_y.shape[1])
                ...
                pred = np.vstack(np.array(y_pred.eval(feed_dict={x_ph: test_x}), dtype = np.float32))
                orig = np.vstack(np.array(test_y, dtype = np.float32))

            sess.close()

        self.time = time.time() - self.time
        self.test_error = 1 - np.mean(np.argmax(pred, axis=1) == np.argmax(orig, axis=1))
